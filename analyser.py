...
def analyse(url: str) -> dict:
    ...
    Schickt HTML-Text + Screenshot an Gemini API
    """
    ...
    safe_url = url.replace('https://', '').replace('http://', '').replace('/', '_').replace('?', '_').replace('&', '_').replace('=', '_')
    screenshot_path = f"output/{safe_url}.png"
    try:
    ...
    # Modell wählen
    model = genai.GenerativeModel('gemini-1.5-flash') # Flash ist schnell und gut für Analyse
    ...
    prompt_parts = [
        f"Hier ist der HTML-Text der Website:\n\n{text_content}",
        system_prompt
    ]
