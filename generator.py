...
def generate(url: str, branche: str) -> str:
    ...
    # 2. KI für Website-Generierung
    api_key_bp = os.getenv("BIG_PICKLE_API_KEY")
    api_key_oa = os.getenv("OPENAI_API_KEY")
    api_key_gm = os.getenv("GEMINI_API_KEY")
    ...
    try:
        if api_key_bp:
            client = OpenAI(api_key=api_key_bp, base_url="https://opencode.ai/zen/v1")
            response = client.chat.completions.create(
                model="big-pickle",
                messages=[{"role": "system", "content": "Du bist ein erfahrener Webdesigner."}, {"role": "user", "content": prompt}],
                max_tokens=4000
            )
