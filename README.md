...
### Kostenlose Version (Free Mode)
WebScore bietet einen **Free Mode**, der ohne kostenpflichtige APIs (Google Places, OpenAI) auskommt:
- **Suche**: Nutzt OpenStreetMap (Overpass API) statt Google Places.
- **KI**: Nutzt Google Gemini (kostenloser Tier verfügbar) für Analyse und Website-Generierung.
- **Aktivierung**: Einfach den "Free Mode" Schalter in der GUI umlegen. Es wird dann nur der `GEMINI_API_KEY` benötigt.
